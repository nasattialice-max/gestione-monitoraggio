...
class AthleteHubApp {
  ...
  initPortalUrls() {
    ...
    const rpeUrl = localStorage.getItem('portal_url_rpe') || defaultRpe;
    const recUrl = localStorage.getItem('portal_url_recovery') || defaultRec;
    
    const rpeInput = document.getElementById('config-url-rpe');
    const recInput = document.getElementById('config-url-recovery');
    
    if (rpeInput) rpeInput.value = rpeUrl;
    if (recInput) recInput.value = recUrl;
